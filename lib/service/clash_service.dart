...
class ClashService extends GetxService with TrayListener {
  ...
  Future<ClashService> init() async {
    ...
    // init config yaml
    final _ = SpUtil.getString('yaml', defValue: currentYaml.value)!;
    initializedHttpPort = SpUtil.getInt('http-port', defValue: 12346)!;
    initializedSockPort = SpUtil.getInt('socks-port', defValue: 12347)!;
    initializedMixedPort = SpUtil.getInt('mixed-port', defValue: 12348)!;
    currentYaml.value = _;
    Request.setBaseUrl(clashBaseUrl);
    // init clash
    // kill all other clash clients
    final clashConfigPath = p.join(_clashDirectory.path, "vpnfly");
  ...
  changeCustomTestUrl(url) {
    ...
    customTestUrl = url;
    SpUtil.putString('customTestUrl', url);
    reload();
  }
  ...
  String getSubscriptionLinkByYaml(String yaml) {
    final url = SpUtil.getString('profile_$yaml', defValue: "")!;
  ...
  Future<bool> updateSubscription(String name) async {
    final configName = '$name.yaml';
    final newProfilePath = p.join(_clashDirectory.path, configName);
    final url = SpUtil.getString('profile_$name')!;
    try {
      final uri = Uri.tryParse(url);
      if (uri == null) {
        return false;
  ...
  bool isHideWindowWhenStart() {
    return SpUtil.getBool('boot_window_hide', defValue: false)!;
  }

  Future<bool> setHideWindowWhenStart(bool hide) {
    return SpUtil.putBool('boot_window_hide', hide)!;
  }
